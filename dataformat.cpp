...
bool getValue(uint32_t data, int16_t * zahl) {
  ...
  if ( getDataType(data) == ZF_UINT ) { 
    *zahl = (uint16_t)getValue_ui(data);
    retval = false;
  }
  return retval;
}
...
uint32_t packTransportValue(uint8_t channel, char* value) {
    uint8_t dataType = ZF_CHAR;
    char* pEnd; 
    for (size_t i=0; i<strlen(value); i++) {
	if ( dataType == ZF_CHAR || dataType == ZF_UINT || dataType == ZF_INT ) {
	    if ( value[i] == '.' ) dataType = ZF_FLOAT;
	}
	if ( dataType == ZF_CHAR || dataType == ZF_UINT ) {
	    if ( value[i] == '-' ) dataType = ZF_INT;
	}
    }
    return packTransportValue(channel, value, dataType);
}
