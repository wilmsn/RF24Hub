...
void receive_tn_in(int new_tn_in_socket, struct sockaddr_in * address) {
    ...
    char *message =  (char*) malloc (DEBUGSTRINGSIZE);
    memset(message, 0, DEBUGSTRINGSIZE);
...
/*    if ( verboselevel & VERBOSETELNET) {    
        sprintf(message,"Client %s is connected ...",inet_ntoa (address->sin_addr) );
...
//
		if ( order.hasOrder ) {  // go transmitting if its time to do ..
