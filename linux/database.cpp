...
void Database::initSensor(SensorClass* sensorClass) {
	...
	uint32_t        last_data;
	MYSQL_ROW row;
	sprintf (sql_stmt, "select a.sensor_id, node_id, channel, datatype, fhem_dev, sensor_name, b.last_value, b.last_utime from sensor a left join sensor_im b on a.sensor_id = b.sensor_id");
	debugPrintSQL(sql_stmt);
	mysql_query(mydb, sql_stmt);
	db_check_error(mydb);
	MYSQL_RES *result = mysql_store_result(mydb);
	...
	while ((row = mysql_fetch_row(result))) {
	    ...
	    if ( row[6] != NULL ) last_data = strtof(row[6], &pEnd); else last_data = 0;
	    if ( row[7] != NULL ) last_utime = strtoul(row[7], &pEnd,10); else last_utime = 0;
