...
void Config::processParams(int argc, char* argv[]) {
	...
	while (1) {
		...
		int option_index = 0;
		c = getopt_long (argc, argv, "?dhit:s:c:",long_options, &option_index);
	...
	while ((s = fgets (buff, sizeof buff, fp)) != NULL) {
		...
		s = strtok (buff, "=");
		if (s==NULL) continue;
		else strncpy (name, s, PARAM_MAXLEN);
		s = strtok (NULL, "=");
		if (s==NULL) continue;
		else strncpy (value, s, PARAM_MAXLEN);
		trim (value);
