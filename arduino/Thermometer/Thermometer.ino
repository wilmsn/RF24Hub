...
#if defined(SER_DEBUG)
        Serial.println(F("Enter SendLoop"));
#endif
        delay(RF24SENDDELAY);
        if ( radio.available() ) {
...
//           }
              if (payload.type == MSGTYPORQ1 ) payload.type = MSGTYPORP1;
              if ((payload.flags & 0x01) == 0x01 ) {
                receiveloopcount = eeprom.receiveloopcount;
              }
              radio.stopListening();
#if defined(SER_DEBUG)
              Serial.print(F("Now transmitting Answer "));
#endif
              if (radio.write(&payload,sizeof(payload))) {
                sendloopcount = eeprom.sendloopcount;
#if defined(SER_DEBUG)
...
#if defined(SER_DEBUG)
                Serial.println(F("Wrong Node => Msg dropped!"));
#endif                
          }
        }
