...
void Sensor::addSensor(uint32_t sensor_id, NODE_DATTYPE node_id, uint8_t channel, char* fhem_dev, 
                        uint32_t last_val_utime, float last_val) {
...
uint32_t Sensor::updateLastVal(NODE_DATTYPE node_id, uint8_t channel, float value, uint64_t mymillis) {
    uint32_t retval = 0;
    sensor_t *p_search;
    p_search=p_initial;
    while (p_search) {
        if ( p_search->node_id == node_id && p_search->channel == channel ) {
            if ( mymillis - p_search->last_ts > 1000 ) {
                ...
                if ( verboselevel & VERBOSESENSOR) printf("%ssensor.updateLastVal: S:%u N:%u C:%u V:%f\n", ts(buf), p_search->sensor_id, node_id, channel, value); 
