...
void setup() {
  ...
  httpServer.on("/restart", []() {
    httpServer.send(304, "message/http");
    ESP.restart();
  });
  httpServer.on("/console", []() {
    httpServer.send(200, "text/plain", logger.printBuffer());
  });
...
void fill_sysinfo2(char* mystr) {
  snprintf (mystr, INFOSIZE, "{\"Freespace\":\"%0.0fKB\", \"Sketchsize\":\"%0.0fKB\", \"FlashSize\":\"%dMB\", \"FlashFreq\":\"%dMHz\", \"CpuFreq\":\"%dMHz\", \"Vcc\":\"%.2fV\"}",
            ESP.getFreeSketchSpace() / 1024.0, ESP.getSketchSize() / 1024.0, (int)(ESP.getFlashChipSize() / 1024 / 1024),
            (int)(ESP.getFlashChipSpeed() / 1000000), (int)(F_CPU / 1000000), (float)ESP.getVcc() / 1000.0 );
}
